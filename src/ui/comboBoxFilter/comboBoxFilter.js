...
export class comboBoxFilter extends Component {
    ...
    render() {
        const {
            max, touched, disabled, invalid, error, scrollTo, name, parentId, onChange, onBlur, onKeyPress, onSelect,
            value, id, idIcon
        } = this.props;
                ...
                <div className={`styleWidthComponents ui dropdown search selection fluid ${name} ${_disabled}`}
                    ...
                    <div className="ui icon input" style={{ width: "100%" }}>
                        ...
                        <i disabled={_disabled} className={`search icon ${_disabled}`}
                            id={idIcon === undefined ? "iconSearchUserFilter" : idIcon} onClick={() => onKeyPress(e)}/>
                    </div>
