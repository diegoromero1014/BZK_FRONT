export const CLEAR_COMMENTS = 'CLEAR_COMMENTS';
export const ADD_COMMENT_LIST = 'ADD_COMMENT_LIST';
export const CREATE_COMMENT = 'CREATE_COMMENT';
export const GET_CURRENT_COMMENTS_LIST = 'GET_CURRENT_COMMENTS_LIST';
export const FILL_COMMENTS = 'FILL_COMMENTS';
export const GET_COMMENTS_BY_REPORT_ID = 'GET_COMMENTS_BY_REPORT_ID';

export const ERROR_COMMENT_LENGTH = 'El comentario no tiene ningún contenido.';
export const NO_NOTES_MESSAGE = 'Sin notas asociadas';
export const TASK_COMMENT_HELP_MESSAGE = 'Señor usuario, puede etiquetar a los usuarios que desee informar de la tarea y les llegara un correo electrónico. Ej: @Usuariored.';
export const MAX_LENGTH_USER_TASK_COMMENT = 1000;