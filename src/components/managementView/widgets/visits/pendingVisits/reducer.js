...
const pendingVisits = (state = initialState, action = {}) => {
	...
	if (action.type == REQUEST_PENDING_VISITS) {
		...
		const newRows = newState.rows.map(element => element = Object.assign({}, element, { visitTime: mapDateValueFromTaskByFormat(element.visitTime, REVIEWED_DATE_FORMAT_HOUR) }))
