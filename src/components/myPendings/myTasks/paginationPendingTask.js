...
class PaginationPendingTask extends Component {
    ...
    _handleFindPendingtask(limInf) {
        const {tasksByUser, myPendingsReducer, showLoading} = this.props;
    ...
    render() {
        ...
        const page = myPendingsReducer.get('page');
        let firstPage = 1;
        if (page > 4) {
            firstPage = page - 3;
        }
        const rowCount = myPendingsReducer.get('rowCount');
