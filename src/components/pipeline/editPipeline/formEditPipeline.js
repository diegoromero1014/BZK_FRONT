...
import {SAVE_DRAFT, SAVE_PUBLISHED, OPTION_REQUIRED, VALUE_REQUIERED, DATE_FORMAT, DATETIME_FORMAT,
...
import {PROPUEST_OF_BUSINESS, POSITIVE_INTEGER, INTEGER, REAL, LAST_PIPELINE_REVIEW, CURRENCY_LABEL_COP,
	CURRENCY_COP, CURRENCY_LABEL_OTHER_OPTION, LINE_OF_BUSINESS_LEASING, ORIGIN_PIPELIN_BUSINESS} from '../constants';
...
import {editBusiness} from '../business/actions';
...
export default function createFormPipeline(name, origin, pipelineBusiness, functionCloseModal){
  ...
  var nameCurrency = _.uniqueId('currency_');
	    ...
	    */
		...
		_submitEditPipeline() {
			const {initialValues, fields: {idUsuario, value, commission, roe, termInMonths, businessStatus,
				...
				estimatedDisburDate, entity, contract}, createEditPipeline, changeStateSaveData, editBusiness} = this.props;
			var idPipeline;
			if(origin === ORIGIN_PIPELIN_BUSINESS){
				idPipeline = pipelineBusiness.uuid;
			}else{
		...
		componentWillMount() {
			...
			const {clientInformacion, getMasterDataFields, getPipelineCurrencies, getClientNeeds,
				...
				business, product, priority, registeredCountry, startDate, client, documentStatus}} = this.props;
        ...
        <div>
          {origin !== ORIGIN_PIPELIN_BUSINESS && <HeaderPipeline />}
					<form onSubmit={handleSubmit(this._submitEditPipeline)} onKeyPress={val => formValidateKeyEnter(val, reducerGlobal.get('validateEnter'))} className="my-custom-tab"
              ...
              <Row style={{ padding: "5px 10px 0px 20px" }}>
							...
							<Col style={origin === ORIGIN_PIPELIN_BUSINESS ? {display:"none"} : {}} xs={2} sm={2} md={2} lg={2}>
						...
						<Row style={origin === ORIGIN_PIPELIN_BUSINESS ? {display:"none"} : { padding: "20px 23px 20px 20px" }}>
							<Col xs={12} md={12} lg={12}>
								<div style={{ fontSize: "25px", color: "#CEA70B", marginTop: "5px", marginBottom: "5px" }}>
									<div className="tab-content-row" style={{ borderTop: "1px dotted #cea70b", width: "100%", marginBottom: "10px" }} />
									<i className="book icon" style={{ fontSize: "18px" }} />
						...
						<Row style={origin === ORIGIN_PIPELIN_BUSINESS ? {display:"none"} : { padding: "10px 10px 0px 20px" }}>
							<Col xs={6} md={3} lg={3}>
						...
						<Row style={origin === ORIGIN_PIPELIN_BUSINESS ? {display:"none"} : { padding: "5px 10px 0px 20px" }}>
							<Col xs={6} md={3} lg={3}>
						...
						<div style={origin !== ORIGIN_PIPELIN_BUSINESS ? { position: "fixed", border: "1px solid #C2C2C2", bottom: "0px", width: "100%", marginBottom: "0px", backgroundColor: "#F8F8F8", height: "50px", background: "rgba(255,255,255,0.75)" } : {display: "none"}}>
	...
	function mapStateToProps({clientInformacion, selectsReducer, contactsByClient, pipelineReducer,
		reducerGlobal, navBar, pipelineBusinessReducer}, pathParameter) {
