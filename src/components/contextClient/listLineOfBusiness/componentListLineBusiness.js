...
class ComponentListLineBusiness extends Component {
    ...
    render() {
        const { contextLineBusiness, participation, experience, showForm, fnShowForm, clientInformacion } = this.props;
        const listParticipation = clientInformacion.get('listParticipation');
        return (
            ...
            <Col xs={6} md={2} lg={1}>
                <button className="btn btn-secondary" disabled={showForm} type="button" title="Agregar línea de negocio"
                    onClick={() => fnShowForm(true)} style={showForm ? { marginLeft: '10px', cursor: 'not-allowed' } : { marginLeft: '10px' }}>
