...
class TabComponent extends Component {
    ...
    componentWillMount() {
        const { navBar, consultModulesAccess, showLoading, swtShowMessage } = this.props;
                        ...
                        <div>
                            <Menu pointing secondary>
                                {_.get(navBar.get('mapModulesAccess'), MODULE_SHAREHOLDERS) &&
                                    < Menu.Item name="Accionistas" active={tabActive === TAB_SHAREHOLDER} onClick={this._handleItemClick.bind(this, TAB_SHAREHOLDER)} />
                                }
                                {_.get(navBar.get('mapModulesAccess'), MODULE_BOARD_MEMBERS) &&
                                    <Menu.Item name="Miembros de junta" active={tabActive === TAB_BOARD_MEMBERS} onClick={this._handleItemClick.bind(this, TAB_BOARD_MEMBERS)} />
                                }
                            </Menu>
                            <Segment>
                                {_.get(navBar.get('mapModulesAccess'), MODULE_SHAREHOLDERS) && tabActive === TAB_SHAREHOLDER && <Shareholder />}
                                {_.get(navBar.get('mapModulesAccess'), MODULE_BOARD_MEMBERS) && tabActive === TAB_BOARD_MEMBERS && <BoardMembers />}
                            </Segment>
                        </div>
                    }
                </div>
