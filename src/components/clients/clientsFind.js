...
const fields = ["team", "certificationStatus", "linkedStatus"];
...
class ClientsFind extends Component {
    ...
    componentWillMount() {
        if (window.localStorage.getItem('sessionToken') === "" || window.localStorage.getItem('sessionToken') === undefined) {
            ...
            getMasterDataFields([constants.LINKED_STATUS]).then((data) => {
                if (_.get(data, 'payload.data.messageHeader.status') === SESSION_EXPIRED) {
                    redirectUrl("/login");
                }
            });
    ...
    _handleClientsFind() {
        const {fields: {certificationStatus, team, linkedStatus}} = this.props;
        const {clientsFindServer, clientR, changePage} = this.props;
        clientsFindServer(clientR.get('keyword'), 0, NUMBER_RECORDS, certificationStatus.value, team.value, linkedStatus.value).then((data) => {
    ...
    render() {
        var clientItems = [];
        const {fields:{team, certificationStatus, linkedStatus}, handleSubmit, navBar, reducerGlobal} = this.props;
